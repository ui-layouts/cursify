...
export const SpringyCursor: React.FC<SpringyCursorProps> = ({ emoji = "⚽", wrapperElement }) => {
  ...
  useEffect(() => {
    ...
    function springForce(i: number, j: number, spring: Vec) {
      let dx = particlesRef.current[i].position.x - particlesRef.current[j].position.x;
      let dy = particlesRef.current[i].position.y - particlesRef.current[j].position.y;
  ...
  return (
    ...
    >
      ...
      <ComponentCard
        ...
        description="A React component that enhances cursor movement with smooth spring animations."
      >
        <LivePreviewCard className="min-h-screen">
          <SpringyCursor emoji="🌟" />
        </LivePreviewCard>
      </ComponentCard>
      ...
      >
        <div className="space-y-4">
          ...
          <CodeExample
            title="Create SpringCursor.tsx Component"
            code={codeToDisplay}
            fileName="./SpringCursor.tsx"
            badgeText="TSX"
          />
